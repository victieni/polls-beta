...
export const TabsList = withUniwind(({ children, style }: TabsListProps) => {
...
export const TabsTrigger = withUniwind(
	({
		children,
		value,
		disabled = false,
		...
		const triggerTextStyle: TextStyle = {
			fontSize: FONT_SIZE,
			fontWeight: "500",
			color: isActive ? primaryForeground : mutedForegroundColor,
			textAlign: "center",
			...textStyle,
		};
...
export const TabsContent = withUniwind(
	({ children, value, style }: TabsContentProps) => {
		const {
			activeTab,
			enableSwipe,
			orientation,
